...
export function demandViews(h: H) {
  ...
  function portal(q: URLSearchParams): View {
    ...
    const accessCard = card("Your access", `<div class="wash-tile"><small class="lbl" style="margin-top:0">Membership</small><b class="big">${esc(money(subAmount, subCur))}</b><span class="dim">a month · ${agency ? "agency access" : "client access"}</span></div>
      <ul class="rows tight"><li><span>Search the network</span><small class="dim">every expert who has agreed to be found</small></li><li><span>Anonymised profiles</span><small class="dim">capability, region, availability, work rights, trust</small></li><li><span>Unlock a profile</span><small class="dim">we ask them, then share everything they agreed to</small></li><li><span>Fees on a placement</span><small class="dim">on top, and only when someone starts</small></li></ul>
      ${forced ? "" : `<div class="row end">${btn("Adjust commission", `data-act="adjustTerms" data-id="${acc.id}"`, "ghost sm")}</div>`}`,
      { desc: "It is a trusted network, not a job board. The subscription is for access to people we actually know." });
    ...
    const html = `${hero}<div class="grid-3"><div class="col-2 stack">${searchCard}${submit}${briefCards}</div><div class="stack">${side}</div></div>`;
