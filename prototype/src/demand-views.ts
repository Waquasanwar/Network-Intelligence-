...
export function demandViews(h: H) {
  ...
  /** Nobody reaches a client or agency view without having agreed to it. Withdrawal is immediate. */
  const shareable = (s: ShortlistItem) => { const p = C().person(s.personId); return !!p && C().privacy.canShareAnonymised(p); };
  const visibleTo = (items: ShortlistItem[]) => items.filter((s) => D().PORTAL_VISIBLE.includes(s.decision) && shareable(s));
  ...
  /** What a client or agency may be told about why we trust this person. No names but our own. */
  const partnerCred = (p: Person) => {
    const S = C().S();
    const vs = C().vouchesOf(p.id);
    const rels = C().relsOf(p.id);
    const metRel = rels.find((r: any) => r.metInPerson);
    const badges = C().credibility.credibilityForPartner({
      relationships: rels.map((r: any) => ({ workedTogether: r.workedTogether, wouldWorkTogetherAgain: r.wouldWorkTogetherAgain })),
      metInPerson: metRel ? { by: "", when: null } : null,
      personallyReferredBy: vs.filter((v: any) => v.voucherKind === "USER" && v.wouldRecommend).map(() => ""),
      usedByUs: p.usedByAmana, engagements: S.team.filter((t) => t.personId === p.id).length || undefined, onBench: p.amanaBench,
      vouchCount: vs.length, wouldRecommendCount: vs.filter((v: any) => v.wouldRecommend).length,
      screened: p.screeningStatus === "APPROVED" || !!p.screenedAt, evidenceCount: C().evOf(p.id).length,
    }, S.me.name);
    return `<div class="cred compact">${badges.slice(0, 4).map((b: any) => `<span class="cred-b s-${b.strength}" title="${esc(b.note)}"><ni-icon name="${b.icon}" size="12" tone="${b.strength === "strong" ? "trust" : "mute"}"></ni-icon><b>${esc(b.label)}</b></span>`).join("")}</div>`;
  };

  /** Can they do it, will we vouch for them, can they take it, will they land well. One signal. */
  const suitOf = (s: ShortlistItem, b: StoredBrief) => {
    const p = C().person(s.personId)!;
    const avail: Record<string, number> = { AVAILABLE_NOW: 1, FINISHING_ENGAGEMENT_SOON: 0.85, OPEN_TO_CONVERSATIONS: 0.7, FRACTIONAL_AVAILABILITY: 0.7, QUIETLY_EXPLORING: 0.6, CONTRACT_ONLY: 0.6, SOW_ONLY: 0.6, RIGHT_OPPORTUNITY_ONLY: 0.45, NEEDS_REFRESH: 0.35, TAKING_A_BREAK: 0.2, NOT_LOOKING_KEEP_IN_TOUCH: 0.15, HAPPY_WHERE_I_AM: 0.1 };
    return C().suitability.suitability({
      capabilityScore: s.fitScore, trustScore: C().trustOf(p).score, checks: s.checks,
      fit: C().fit.fitChecks(b.fitTraits ?? [], C().fitOf(p)), evidenceCount: C().evOf(p.id).length,
      workedWith: C().relsOf(p.id).some((r) => r.workedTogether), usedByUs: p.usedByAmana,
      availability: avail[p.availabilityStatus] ?? 0.5,
    });
  };

  /** The deck: one expert, one decision. Quick, but never thoughtless — the reasons are on the card. */
  const deckCard = (s: ShortlistItem, b: StoredBrief, idx: number, total: number) => {
    const p = C().person(s.personId)!; const su = suitOf(s, b); const tr = C().trustOf(p);
    const tone = su.band === "strong" ? "trust" : su.band === "worth a conversation" ? "" : "alert";
    return `<section class="deck-card" data-deck-idx="${idx}"><div class="deck-top"><span class="deck-count">${idx + 1} of ${total}</span><ni-tag tone="${tone || "neutral"}" solid icon="${su.band === "strong" ? "check" : su.band === "no" ? "cross" : "ask"}">${esc(C().suitability.suitabilityLabel(su.band))}</ni-tag></div>
      <div class="deck-head">${h.avatar(p, "xl", su.band === "strong")}<div><h3>${esc(C().full(p))}</h3><p>${esc(p.headline ?? "")}</p><small>${esc([p.currentRole, [p.primaryCity, p.primaryCountry].filter(Boolean).join(", ")].filter(Boolean).join(" · "))}</small></div><ni-trust score="${su.score}" band="suitable" size="86"></ni-trust></div>
      <p class="deck-line">${esc(su.line)}</p>
      <div class="deck-parts">${su.parts.map((x: any) => `<div class="dp"><div class="dp-top"><small>${esc(x.label)}</small><b>${x.score}</b></div><div class="dp-bar"><div style="width:${x.score}%"></div></div><small class="dp-note">${esc(x.note)}</small></div>`).join("")}</div>
      ${su.watchOut ? `<div class="note-amber deck-watch"><small>Watch out</small>${esc(su.watchOut)}</div>` : ""}
      ${checkList(s.checks)}
      <div class="deck-cred">${partnerCred(p)}</div>
      <div class="deck-actions">${btn("Not for this", `data-act="deckPass" data-id="${s.id}"`, "glass lg")}${btn("Propose to the client", `data-act="deckPropose" data-id="${s.id}"`, "primary lg")}${btn("Skip", `data-act="deckSkip" data-id="${s.id}"`, "ghost")}</div>
      <small class="deck-hint">← not for this · → propose · they only ever see an anonymised card until you unlock them</small>
    </section>`;
  };

  const anonEntry = (s: ShortlistItem, revealed: boolean) => { const p = C().person(s.personId)!; const unlocked = !!s.unlocked || revealed; const named = unlocked && C().privacy.canRevealName(p, true); const kind = accountOf(C().S().briefs.find((b) => b.id === s.briefId)!)?.kind ?? "CLIENT"; const x = C().redactForPartner({ ...p, tenantId: "t", evidence: C().evOf(p.id), relationships: C().relsOf(p.id) }); const rights = s.checks.find((c) => c.label === "Work rights"); const band = D().rateBand(p.rateExpectation ?? p.salaryExpectation); const tr = C().trustOf(p); const hl = C().fit.fitHighlights(C().fitOf(p)); return `<li class="anon"><div class="row"><span class="row"><code>${esc(x.ref)}</code>${named ? `<b>${esc(C().full(p))}</b>` : `<b>${esc(x.headlineSummary || p.headline || "Profile")}</b>`}${p.seal ? `<ni-tag tone="trust" solid icon="trust" title="${esc(p.seal.note ?? "")}">Trust seal</ni-tag>` : ""}${unlocked ? `<ni-tag tone="trust" solid icon="check">Unlocked</ni-tag>` : ""}</span>${badge(D().shortlistLabel(s.decision, kind), s.decision === "CLIENT_PASSED" ? "neutral" : s.decision === "PLACED" || s.decision === "INTRODUCED" ? "teal" : "navy", true)}${s.referred ? '<ni-tag tone="trust" solid icon="referral">referred by hand</ni-tag>' : ""}</div><div class="chips">${chip(C().privacy.partnerHidden(p).region ? "region hidden by them" : x.region ?? "region undisclosed")}${chip(`availability: ${x.availabilityBand}`)}${rights ? chip(`work rights: ${rights.state === "met" ? "confirmed" : rights.state === "unmet" ? "sponsorship needed" : "to confirm"}`) : ""}${band && !C().privacy.partnerHidden(p).rateBand ? chip(band) : ""}${chip(s.tier === "meets" ? "meets the brief" : s.tier === "conversation" ? "one point to confirm" : "stretch")}<ni-tag icon="vouch" solid>vouched by ${tr.vouchedBy}</ni-tag><ni-tag icon="trust" tone="${tr.band === "highly trusted" || tr.band === "trusted" ? "trust" : "neutral"}" solid>${esc(C().trust.TRUST_BAND_LABEL[tr.band].toLowerCase())}</ni-tag>${hl.map((t: string) => chip(t)).join("")}</div>${partnerCred(p)}<small class="dim">${esc(x.evidenceSummary)}</small>${unlocked ? `<div class="unlocked"><small class="lbl">Shared with you</small><dl class="kv"><dt>Known for</dt><dd>${esc(p.headline ?? "—")}</dd><dt>Expertise</dt><dd>${esc(p.capabilities.join(", "))}</dd><dt>Sectors</dt><dd>${esc(p.sectors.join(", ") || "—")}</dd><dt>Based</dt><dd>${esc([p.primaryCity, p.primaryCountry].filter(Boolean).join(", ") || "—")}</dd><dt>Work rights</dt><dd>${esc((p.workRights ?? []).join(", ") || "to confirm")}</dd><dt>Availability</dt><dd>${esc(h.L().AVAILABILITY_LABELS[p.availabilityStatus])}</dd><dt>${p.rateExpectation ? "Rate" : "Salary"}</dt><dd>${esc(p.rateExpectation ?? p.salaryExpectation ?? "on application")}</dd>${named ? `<dt>Contact</dt><dd>through us, and only with their say-so</dd>` : ""}</dl>${p.seal ? `<div class="seal"><ni-icon name="trust" size="16" tone="trust"></ni-icon><div><b>Trust seal · ${esc(p.seal.by)}</b><small>${esc(p.seal.note ?? "We stand behind this person.")}</small></div></div>` : ""}</div>` : ""}${s.clientNote ? `<small class="dim">Note from us: ${esc(s.clientNote)}</small>` : ""}${(() => { const pi = C().S().pitches.find((z: any) => z.personId === s.personId && z.briefId === s.briefId); return pi ? `<div class="pitch-quote"><small class="lbl">In their own words</small><p>“${esc(pi.note)}”</p>${pi.relevantWork ? `<small class="dim">${esc(pi.relevantWork)}</small>` : ""}<div class="chips">${pi.route ? chip(h.L().ROUTE_LABELS[pi.route] ?? pi.route) : ""}${pi.availableFrom ? chip(`available ${pi.availableFrom}`) : ""}</div></div>` : ""; })()}</li>`; };
  ...
  function requirements(q: URLSearchParams): View {
    ...
    const html = `<div class="page-head"><div><div class="eyebrow">Demand</div><h1>Requirements</h1><p>Clients, agencies and Amana tell us what they need. The co-pilot structures it, you decide who to propose, and the fee is worked out as you go.</p></div><div class="actions">${btn("＋ New account", 'data-act="newAccount"', "glass")}</div></div>
      ...
      ${card("Accounts", `<ul class="rows">${S.accounts.map((a) => `<li><span><b class="t">${esc(a.name)}</b><small class="sub">${esc(KIND_LABEL[a.kind])} · ${S.briefs.filter((b) => b.accountId === a.id).length} requirements${a.monthlyFee ? ` · ${money(a.monthlyFee, a.currency)}/mo access` : ""}</small></span><span class="row gap-sm">${badge(a.status.toLowerCase(), a.status === "ACTIVE" ? "teal" : "amber")}${btn("Adjust", `data-act="adjustTerms" data-id="${a.id}"`, "ghost sm")}${btn("Remove", `data-act="removeAccount" data-id="${a.id}"`, "ghost sm")}</span></li>`).join("")}</ul>`, { desc: "Who pays, and on what terms.", action: btn("Rate card", 'data-act="goCommercials"', "ghost sm") })}</div></div>`;
  ...
  function requirement(id: string): View {
    ...
    const visible = visibleTo(sl); const fees = feesFor(id);
  ...
  function portal(q: URLSearchParams): View {
    ...
    const proposed = visibleTo(S.shortlist.filter((s) => mine.some((b) => b.id === s.briefId)));
    ...
    const stats = agency
      ? [["Live requirements", open.length, "open with us"], ["Experts you can place", proposed.length, "anonymised"], ["With your client", asked.length + introduced.length, "asked or introduced"], ["Placed", placed.length, "through us"]] as const
      : [["Open roles", open.length, "with us"], ["Experts proposed", proposed.length, "anonymised"], ["Interviews requested", asked.length, "we are asking them"], ["Hires", placed.length, "started"]] as const;
    const hero = `<div class="portal-hero"><div><div class="eyebrow">${forced ? `${KIND_LABEL[acc.kind]} portal` : `${KIND_LABEL[acc.kind]} portal · viewing as`}</div>${forced ? "" : `<form class="row wrap switcher" data-action="portalSwitch"><select name="account" class="lg">${opt(Object.fromEntries(accounts.map((a) => [a.id, `${a.name} · ${KIND_LABEL[a.kind]}`])), acc.id)}</select><button class="btn glass sm" type="submit">Switch</button></form>`}<h1>${esc(acc.name)}</h1><p>${agency ? "Requirements you are working on, and people from our network you can put in front of your client. Names stay with us until they agree to be introduced — so the relationship, and the fee, are protected on both sides." : "The roles you have with us, and the people we would put in front of you. Every card is someone a person in our network has worked with. Names come with their consent, and you pay only when someone starts."}</p></div>
      <div class="hero-stats">${stats.map(([l, v, h]) => `<ni-stat ghost label="${esc(l)}" value="${v}" note="${esc(h)}"></ni-stat>`).join("")}</div></div>`;
    ...
    const split = (() => {
      const perm = rc.agencyPermPct, share = rc.agencyReferralSharePct; const keep = Math.round((100 - share) * 10) / 10;
      return `<div class="wash-tile"><small class="lbl" style="margin-top:0">Your fee to your client</small><b class="big">${perm}%</b><span class="dim">of first-year salary. Of that fee:</span>
        <ni-bar class="split-bar" segments="Our share:${share}:accent|You keep:${keep}:mute" unit="%"></ni-bar></div>
    ...
    const sub = D().subscriptionFor(acc.kind, rc);
    const subAmount = acc.monthlyFee ?? sub.amount;
    const subCur = acc.monthlyFee ? acc.currency : sub.currency;
    /**
     * Membership, fees and contract as one document. They used to be three cards and a separate
     * "adjust commission" drawer, which is three places to look and three things to keep in step.
     */
    const AG = C().agreement;
    const ag = AG.buildAgreement(
      { id: acc.id, name: acc.name, kind: acc.kind, currency: acc.currency, monthlyFee: acc.monthlyFee, createdAt: acc.createdAt },
      S.rateCard,
      (acc.terms as any)?.agreement ?? {},
      { status: acc.status === "ACTIVE" ? "ACTIVE" : "DRAFT", startedAt: acc.createdAt },
    );
    const diffs = AG.departures(ag);
    const accessCard = card("Membership &amp; terms", `<div class="wash-tile"><small class="lbl" style="margin-top:0">${esc(ag.membership.tier)}</small><b class="big">${esc(money(ag.membership.monthly, ag.membership.currency))}</b><span class="dim">a month · ${esc(money(ag.membership.annual, ag.membership.currency))} a year</span></div>
      <p class="ag-sum">${esc(ag.summary)}</p>
      <ul class="rows tight">${ag.fees.slice(0, 3).map((f: any) => `<li><span>${esc(f.label)}</span><small class="dim">${esc(f.value)} — ${esc(f.basis)}</small></li>`).join("")}</ul>
      <div class="row end" style="margin-top:12px"><a class="btn glass sm" href="#/portal?account=${acc.id}&tab=terms">Read the agreement</a>${forced ? "" : btn("Adjust", `data-act="adjustTerms" data-id="${acc.id}"`, "ghost sm")}</div>`,
      { desc: `${esc(ag.meta.reference)} · ${esc(AG.AGREEMENT_STATUS_LABELS[ag.meta.status])}. One agreement: what you pay monthly, what a placement costs, and the terms.` });

    // Search is the thing a subscription buys, so it sits at the top of the portal.
    const q2 = (q.get("find") ?? "").trim();
    const found = q2 ? C().S().people.filter((p) => C().privacy.canShareAnonymised(p) && [p.headline, p.capabilities.join(" "), p.sectors.join(" "), p.primaryCity, p.primaryCountry].join(" ").toLowerCase().includes(q2.toLowerCase())).slice(0, 8) : [];
    const searchCard = card("Search the network", `<form data-action="portalFind" data-account="${acc.id}" class="searchbar"><div class="search"><span>⌕</span><input name="find" value="${esc(q2)}" placeholder="${esc(agency ? "e.g. business analyst Dubai core banking" : "e.g. programme director, banking, Gulf")}" autocomplete="off"></div><button class="btn primary" type="submit">Search</button></form>
      ${q2 ? (found.length ? `<ul class="rows anon-list find-list">${found.map((p) => { const x = C().redactForPartner({ ...p, tenantId: "t", evidence: C().evOf(p.id), relationships: C().relsOf(p.id) }); const tr = C().trustOf(p); const hidden = C().privacy.partnerHidden(p); const band = D().rateBand(p.rateExpectation ?? p.salaryExpectation); return `<li class="anon"><div class="row"><span class="row gap-sm"><code>${esc(x.ref)}</code><b>${esc(x.headlineSummary || "Profile")}</b>${p.seal ? `<ni-tag tone="trust" solid icon="trust">Trust seal</ni-tag>` : ""}</span>${btn("Ask about this expert", `data-act="askAbout" data-id="${p.id}" data-account="${acc.id}"`, "glass sm")}</div><div class="chips">${chip(hidden.region ? "region hidden by them" : x.region ?? "region undisclosed")}${chip(`availability: ${x.availabilityBand}`)}${band && !hidden.rateBand ? chip(band) : ""}${chip(`trust: ${C().trust.TRUST_BAND_LABEL[tr.band].toLowerCase()}`)}</div>${partnerCred(p)}</li>`; }).join("")}</ul>` : '<p class="dim">Nobody matches that yet. Try fewer words, or send us the requirement and we will go and look.</p>') : '<p class="dim">Search by capability, sector or city. You see anonymised profiles; names come only when the person agrees.</p>'}`,
      { desc: "Included in your membership. Everyone here has agreed to be found." });
    ...
    const briefCards = mine.length ? mine.map((b) => { const items = visibleTo(S.shortlist.filter((s) => s.briefId === b.id)); return card(b.title, `${briefChips(b)}${b.questions.length && b.status !== "FILLED" ? `<div class="note-amber"><small>We will confirm with you</small><ul>${b.questions.map((x) => `<li>${esc(x)}</li>`).join("")}</ul></div>` : ""}<div class="terms-line"><b>${agency ? "Our share" : "Terms"}</b> ${esc(termsText(b))} ${b.termsAccepted ? badge("accepted", "teal", true) : btn("Accept terms", `data-act="acceptTerms" data-id="${b.id}"`, "primary sm")}</div><small class="lbl">${agency ? "Experts you can place" : "Experts we propose"}</small>${items.length ? `<ul class="rows anon-list">${items.map((s) => `${anonEntry(s, ["INTRODUCED", "PLACED"].includes(s.decision))}${s.decision === "PROPOSED" ? `<li class="row portal-actions">${btn(ctaLabel, `data-act="portalInterest" data-id="${s.id}"`, "primary sm")}${btn("Not for us", `data-act="portalPass" data-id="${s.id}"`, "ghost sm")}<small class="dim">Their name stays hidden. We ask them first.</small></li>` : s.decision === "CLIENT_INTERESTED" ? `<li class="row portal-actions"><small class="dim">${esc(waitLabel)}${b.termsAccepted ? "" : ". Accept the terms above so we can proceed"}.</small></li>` : ""}`).join("")}</ul>` : `<span class="dim">${b.status === "NEW" ? "Received. We are reading it now." : "Searching the network. You will see anonymised cards here first."}</span>`}`, { desc: `${D().BRIEF_STATUS_LABELS[b.status]} · sent ${rel(b.createdAt)}` }); }).join("") : empty(agency ? "No requirements yet" : "No roles yet", "Send your first one above.");

    const tab = q.get("tab") ?? "";
    const agreementPage = `<div class="sect-head"><div><h2>Membership &amp; terms</h2><p>One agreement: what you pay every month, what a placement costs, and the contract. ${esc(ag.meta.reference)} · version ${ag.meta.version} · ${esc(AG.AGREEMENT_STATUS_LABELS[ag.meta.status])}.</p></div>${forced ? "" : btn("Adjust this agreement", `data-act="adjustTerms" data-id="${acc.id}"`, "glass sm")}</div>
      <div class="grid-3"><div class="col-2 stack">
        ${card("1 · Membership", `<div class="wash-tile"><small class="lbl" style="margin-top:0">${esc(ag.membership.tier)}</small><b class="big">${esc(money(ag.membership.monthly, ag.membership.currency))}</b><span class="dim">a month, billed in advance · ${esc(money(ag.membership.annual, ag.membership.currency))} a year</span></div>
          <ul class="rows tight">${ag.membership.includes.map((x: string) => `<li><span>${esc(x)}</span></li>`).join("")}</ul>`, { desc: "What the monthly fee opens. Cancel with notice; see the contract below." })}
        ${card("2 · What a placement costs", `<ul class="fee-lines">${ag.fees.map((f: any) => `<li><div class="fl-head"><b>${esc(f.label)}</b><span class="fl-val ${f.overridden ? "adj" : ""}">${esc(f.value)}${f.overridden ? `<small>was ${esc(f.cardValue)}</small>` : ""}</span></div><p>${esc(f.basis)}</p></li>`).join("")}</ul>`, { desc: "Payable on the person's start date and never before it.", flush: true })}
        ${card("3 · The contract", `<ol class="clauses">${ag.contract.map((c: any) => `<li><b>${esc(c.heading)}</b><p>${esc(c.body)}</p></li>`).join("")}</ol>`, { desc: "Nine clauses, in plain words. This is the whole of it.", flush: true })}
      </div><div class="stack">
        ${diffs.length ? card("Agreed specifically with you", `<ul class="rows">${diffs.map((d: string) => `<li class="col"><small>${esc(d)}</small></li>`).join("")}</ul>`, { desc: "Where this agreement differs from our standard terms.", cls: "accent" }) : card("Standard terms", '<p class="dim">Nothing here differs from our standard terms. Every number is the published rate.</p>')}
        ${card("At a glance", `<ul class="rows"><li><span>Monthly</span><b>${esc(money(ag.membership.monthly, ag.membership.currency))}</b></li><li><span>A year</span><b>${esc(money(ag.membership.annual, ag.membership.currency))}</b></li><li><span>Reference</span><code>${esc(ag.meta.reference)}</code></li><li><span>Started</span><span>${esc(ag.meta.startedAt ? new Date(ag.meta.startedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "—")}</span></li></ul>`)}
        ${card("If you want to leave", `<p class="dim">Thirty days' notice, by email, no conversation required. Fees already earned on a placement survive; nothing else does. We will export anything you want to take with you.</p>`)}
      </div></div>`;
    const html = tab === "terms" ? `${hero}${agreementPage}` : `${hero}<div class="grid-3"><div class="col-2 stack">${searchCard}${submit}${briefCards}</div><div class="stack">${side}</div></div>`;
  ...
  /** One decision from the deck. Same consequences as the form below it — just faster. */
  async function deckDecide(itemId: string, decision: "PROPOSED" | "NOT_FOR_THIS") {
    const S = C().S(); const s = S.shortlist.find((x) => x.id === itemId)!; const b = S.briefs.find((x) => x.id === s.briefId)!; const p = C().person(s.personId)!;
    const now = C().nowISO();
    await C().commit("shortlist", { ...s, decision, updatedAt: now }, { action: `shortlist.${decision.toLowerCase()}`, entityType: "Brief", entityId: b.id, detail: `${C().full(p)} · ${b.title}` });
    if (decision === "PROPOSED" && ["NEW", "QUALIFYING", "SEARCHING"].includes(b.status)) await C().commit("briefs", { ...b, status: "SHORTLISTED", updatedAt: now });
    C().toast(decision === "PROPOSED" ? `${p.firstName} proposed — anonymised until you unlock them` : `Noted. ${p.firstName} is not for this one`, decision === "PROPOSED" ? "teal" : "neutral");
    C().render();
  }
  ...
  const actions: Record<string, (el: HTMLElement) => Promise<void> | void> = {
    ...
    /**
     * Unlock one expert for this account. Until now they have seen an anonymised card; unlocking
     * shares everything the expert agreed to share — and the name too, where they consented to it.
     */
    async unlockExpert(el) {
      const S = C().S(); const s = S.shortlist.find((x) => x.id === el.dataset.id)!; const b = S.briefs.find((x) => x.id === s.briefId)!; const p = C().person(s.personId)!;
      const acc = accountOf(b)!; const now = C().nowISO();
      if (!C().privacy.canShareAnonymised(p)) { C().toast(`${p.firstName} has not agreed to be shown outside the network`, "amber"); return; }
      await C().commit("shortlist", { ...s, unlocked: true, unlockedAt: now, decision: s.decision === "PROPOSED" ? "CLIENT_INTERESTED" : s.decision, updatedAt: now }, { action: "identity.reveal", entityType: "Brief", entityId: b.id, detail: `${C().full(p)} unlocked for ${acc.name}` });
      const fee = C().S().rateCard.unlockFee;
      if (fee > 0) await C().commit("fees", { id: C().uid(), briefId: b.id, accountId: acc.id, personId: p.id, model: "INTRODUCTION_FEE", basis: `Profile unlock · ${C().full(p)}`, gross: fee, ourTake: fee, currency: C().S().rateCard.subscriptionCurrency, status: "AGREED", createdAt: now, updatedAt: now } as FeeLine);
      C().toast(`${acc.name} can now see ${C().privacy.canRevealName(p, true) ? C().full(p) : "the full profile"}`); C().render();
    },
    async lockExpert(el) {
      const s = C().S().shortlist.find((x) => x.id === el.dataset.id)!;
      await C().commit("shortlist", { ...s, unlocked: false, updatedAt: C().nowISO() }, { action: "identity.reveal", entityType: "Brief", entityId: s.briefId, detail: "locked again" });
      C().toast("Locked again — back to the anonymised card"); C().render();
    },
    /** A seal is us putting our own name on someone. It is granted by hand and can be taken back. */
    sealExpert(el) {
      const p = C().person(el.dataset.id!)!; const S = C().S();
      C().openDrawer(`Trust seal for ${C().full(p)}`, "Our own mark on this person. Clients see it as a guarantee from us, so it is given by hand and never automatically.", raw(
        `${field("What the seal says", input("note", "", p.seal?.note ?? "Known personally and vouched for by Amana Network"), "Shown to clients under the seal")}
         <div class="wash-tile"><b>This means we stand behind them</b><span class="dim">Only give it to someone you would put in front of your own client without hesitating. You can remove it at any time.</span></div>
         ${p.seal ? check("remove", "Remove the seal instead", false) : ""}`
      ), async (fd: FormData) => {
        const now = C().nowISO();
        const remove = fd.get("remove") === "on";
        await C().commit("people", { ...p, seal: remove ? null : { by: S.me.name, at: p.seal?.at ?? now, note: String(fd.get("note") || "") || null }, updatedAt: now }, { action: "person.update", entityType: "Person", entityId: p.id, detail: remove ? "trust seal removed" : "trust seal granted" });
        C().closeDrawer(); C().toast(remove ? "Seal removed" : `Sealed — ${p.firstName} now carries our mark`); C().render();
      }, { submitLabel: p.seal ? "Save" : "Give the seal" });
    },
    /** A subscriber asking about one expert they found. It lands in the owner's requirements queue. */
    async askAbout(el) {
      const S = C().S(); const p = C().person(el.dataset.id!)!; const acc = S.accounts.find((a) => a.id === el.dataset.account)!;
      const x = C().redactForPartner({ ...p, tenantId: "t", evidence: C().evOf(p.id), relationships: C().relsOf(p.id) });
      const now = C().nowISO();
      const parsed: Brief = D().parseBrief(`${p.currentRole ?? "Expert"} in ${[p.primaryCity, p.primaryCountry].filter(Boolean).join(", ") || "the network"}`);
      const model: FeeModel = D().defaultFeeModel(p.engagementPreferences[0] ?? null, acc.kind);
      const b: StoredBrief = { ...parsed, id: C().uid(), title: `Enquiry · ${x.ref}`, rawText: `${acc.name} asked about ${x.ref} from a search.`, accountId: acc.id, submittedVia: "PORTAL", status: "NEW", capabilities: p.capabilities.slice(0, 6), headcount: 1, questions: [], assumptions: ["Came from a search, not a written requirement."], terms: { ...D().defaultTerms(S.rateCard, model), ...(acc.terms ?? {}) }, termsAccepted: false, createdAt: now, updatedAt: now };
      await C().commit("briefs", b, { action: "portal.brief_submitted", entityType: "Brief", entityId: b.id, detail: `${acc.name} asked about ${x.ref}` });
      await C().commit("shortlist", { id: C().uid(), briefId: b.id, personId: p.id, fitScore: 0, fitExplanation: `${acc.name} found them in search and asked about them.`, dimensions: [], uncertainty: [], checks: D().hardChecks(b, C().toBriefPerson(p)), tier: "conversation", decision: "CANDIDATE", createdAt: now, updatedAt: now } as ShortlistItem);
      C().toast("Asked. We will come back to you — and we ask them before sharing anything."); C().render();
    },
    /** Commission is a conversation, not a constant: set it per account, whenever. */
    adjustTerms(el) {
      const S = C().S(); const acc = S.accounts.find((a) => a.id === el.dataset.id)!; const rc = S.rateCard;
      const t = acc.terms ?? {}; const sub = D().subscriptionFor(acc.kind, rc);
      C().openDrawer(`Commercials · ${acc.name}`, "The default for this account. Any single requirement can still override it.", raw(
        `${field("Fee model", select("model", opt(D().FEE_MODEL_LABELS, t.model ?? D().defaultFeeModel(null, acc.kind))))}
         <div class="grid-2">${field("Our %", input("pct", 'type="number" step="0.5" min="0"', String(t.pct ?? "")))}${field("Flat fee", input("flat", 'type="number" min="0"', String(t.flat ?? "")), "Used for introductions and unlocks")}</div>
         <div class="grid-2">${field("Monthly access", input("monthlyFee", 'type="number" min="0"', String(acc.monthlyFee ?? sub.amount)), `${KIND_LABEL[acc.kind]} standard is ${money(sub.amount, sub.currency)}`)}${field("Currency", select("currency", opt({ GBP: "GBP", AED: "AED", SAR: "SAR", USD: "USD", EUR: "EUR" }, acc.currency)))}</div>
         <div class="wash-tile"><b>Ad hoc is fine</b><span class="dim">Deals get done on the phone. Put whatever you agreed here and every estimate for this account follows it.</span></div>`
      ), async (fd: FormData) => {
        const model = String(fd.get("model")) as FeeModel;
        const next: Account = { ...acc, currency: String(fd.get("currency")), monthlyFee: Number(fd.get("monthlyFee") || 0) || null, terms: { model, pct: Number(fd.get("pct") || 0) || null, flat: Number(fd.get("flat") || 0) || null, currency: String(fd.get("currency")) } };
        await C().commit("accounts", next, { action: "account.create", entityType: "Account", entityId: acc.id, detail: `commercials changed · ${D().FEE_MODEL_LABELS[model]}` });
        C().closeDrawer(); C().toast(`${acc.name}'s commercials updated`); C().render();
      }, { submitLabel: "Save commercials" });
    },
    async deckPropose(el) { await deckDecide(el.dataset.id!, "PROPOSED"); },
    async deckPass(el) { await deckDecide(el.dataset.id!, "NOT_FOR_THIS"); },
    async deckSkip(el) { const s = C().S().shortlist.find((x) => x.id === el.dataset.id)!; await C().commit("shortlist", { ...s, decision: "SHORTLISTED", updatedAt: C().nowISO() }); C().render(); },
    setupClient() { accountFlow("CLIENT"); },
    setupAgency() { accountFlow("AGENCY"); },
    /** Remove a client or agency: take away portal access and its record, with everything that
     *  goes with it. Owner-only in practice — the portal roles never reach this view. */
    async removeAccount(el) {
      const S = C().S(); const a = S.accounts.find((x) => x.id === el.dataset.id); if (!a) return;
      const briefs = S.briefs.filter((b) => b.accountId === a.id);
      const fees = S.fees.filter((f) => f.accountId === a.id);
      const msg = `Remove ${a.name}?\n\nThis ends their portal access and deletes the account, ${briefs.length} requirement${briefs.length === 1 ? "" : "s"} and ${fees.length} fee line${fees.length === 1 ? "" : "s"}. Anything already placed stays in the record. This cannot be undone.`;
      if (typeof confirm === "function" && !confirm(msg)) return;
      for (const b of briefs) { for (const it of S.shortlist.filter((x) => x.briefId === b.id)) await C().removeDoc("shortlist", it.id); await C().removeDoc("briefs", b.id); }
      for (const f of fees) await C().removeDoc("fees", f.id);
      await C().removeDoc("accounts", a.id);
      C().logAudit("account.remove", "Account", a.id, a.name);
      C().toast(`${a.name} removed`, "neutral");
      C().render();
    },
  ...
  const forms: Record<string, (fd: FormData, form: HTMLFormElement) => Promise<void> | void> = {
    ...
    portalFind(fd, form) { const q = new URLSearchParams(); q.set("account", form.dataset.account ?? ""); const t = String(fd.get("find") || "").trim(); if (t) q.set("find", t); location.hash = `#/portal?${q}`; C().render(); },
