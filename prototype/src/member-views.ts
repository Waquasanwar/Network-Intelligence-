...
export function memberViews(h: H) {
  ...
  const screenLabel = (p: Person) => { const [l, t] = SCREEN_STATUS[p.screeningStatus ?? (p.screenedAt ? "APPROVED" : "NONE")] ?? SCREEN_STATUS.NONE; return badge(l, t, true); };
  ...
  const trustCard = (p: Person, opts: { canVouch?: boolean; self?: boolean } = {}) => { const t = C().trustOf(p); return card("Trust", `<div class="trust-head">${ring(t.score, "of 100", 72)}<div><b class="band">${esc(C().trust.TRUST_BAND_LABEL[t.band])}</b><p class="dim">${opts.self ? "How many people stand behind you, and how well the network knows you." : `How many people stand behind ${esc(p.firstName)}, and how well the network knows ${p.firstName.endsWith("s") ? "them" : "them"}. Network confidence, not a judgement.`}</p><div class="chips">${chip(`vouched by ${t.vouchedBy}`)}${p.screenedAt || p.screeningStatus === "APPROVED" ? chip("screened") : chip("not screened")}${C().relsOf(p.id).some((r: Relationship) => r.workedTogether) ? chip("worked with directly") : ""}</div></div></div><ul class="trust-bars">${t.breakdown.map((b: any) => `<li><span>${esc(b.label)}</span><div class="bar"><div class="fill ${b.points < 0 ? "neg" : ""}" style="width:${b.max ? Math.min(100, Math.max(0, (b.points / b.max) * 100)) : 100}%"></div></div><b class="${b.points < 0 ? "bad" : ""}">${b.points > 0 ? "+" : ""}${b.points}</b><small>${esc(b.note)}</small></li>`).join("")}</ul>`, { action: opts.canVouch ? btn("Vouch", `data-act="vouch" data-id="${p.id}"`, "glass sm") : undefined }); };
  const fitCard = (p: Person, opts: { self?: boolean; traits?: string[] } = {}) => { const rows = C().fitOf(p); const any = rows.some((r: any) => r.combined !== null); return card(opts.self ? "How you work" : "How they work", any ? `<ul class="fit-rows">${rows.map((r: any) => `<li class="${opts.traits?.includes(r.key) ? "wanted" : ""}"><span class="lbl-row"><b>${esc(r.label)}</b>${opts.traits?.includes(r.key) ? badge("asked for", "navy", true) : ""}</span><div class="fit-scale"><small>${esc(r.low)}</small><div class="fit-track">${r.self !== null ? `<i class="self" style="left:${((r.self - 1) / 4) * 100}%" title="Self-assessed ${r.self}/5"></i>` : ""}${r.peers !== null ? `<i class="peers" style="left:${((r.peers - 1) / 4) * 100}%" title="Observed by ${r.peerCount}: ${r.peers}/5"></i>` : ""}</div><small>${esc(r.high)}</small></div><em>${r.combined !== null ? `${r.combined}/5` : "—"}${r.peerCount ? `<small>${r.peerCount} observed</small>` : r.self !== null ? "<small>self</small>" : ""}</em></li>`).join("")}</ul><p class="legend"><i class="self"></i> self-assessed in screening <i class="peers"></i> observed by people who vouched</p>` : `<span class="dim">${opts.self ? "Complete the screening to build your working-style profile." : "Not assessed yet. The screening call captures this, and each vouch adds what people have seen."}</span>`, { desc: opts.self ? "Your working style: assertiveness, conflict handling, political and commercial awareness. Clients see the strengths, never the numbers." : "Working style and attitude, so we know where they will land well. Never a pass or fail." }); };
