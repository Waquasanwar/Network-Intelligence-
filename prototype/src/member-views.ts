...
export function memberViews(h: H) {
  ...
  function member(q: URLSearchParams): View {
    ...
    if (tab === "opportunities") body = `<div class="sect-head"><div><h2>Opportunities open to members</h2><p>Anonymised. Pitch with your own profile, or refer someone you would put your name behind. You are only ever named to a client with your consent.</p></div></div>${open.length ? `<div class="stack">${open.map(oppCard).join("")}</div>` : empty("Nothing open right now", "When a requirement is opened to members, it appears here.")}`;
    else if (tab === "refer") body = `<div class="grid-3"><div class="col-2 stack">${card("Refer someone", `<form data-action="refer" class="stack"><p class="dim">Only people you would genuinely stand behind. Your referral is anonymous to clients; we tell the person you sent them only if you say we can.</p><div class="grid-2">${field("Full name", input("name", "required"), undefined, true)}${field("Email or LinkedIn", input("email", 'placeholder="so we can reach them"'))}</div>${field("How you know them and what you have seen them do", textarea("context", 'rows="3" required placeholder="e.g. Ran the requirements workstream for me on the payments hub. Precise, calm, well liked by the business."'), undefined, true)}<div class="grid-2">${field("For an open opportunity?", select("briefId", opt(Object.fromEntries(open.map((b: StoredBrief) => [b.id, `${b.roles[0] ?? "Requirement"} · ${b.locations[0] ?? ""}`])), "", "— general referral —")))}${field("Anything else", input("note"))}</div>${check("tell", "You may tell them I referred them", true)}<div class="row end"><button class="btn primary" type="submit">Send referral</button></div></form>`, { desc: "This is what makes the network compound." })}</div><div class="stack">${card("Your referrals", myRefs.length ? `<ul class="rows">${myRefs.map((r: Referral) => `<li class="col"><div class="row"><b>${esc(r.name)}</b>${`<ni-tag icon="referral" tone="${r.status === "ACCEPTED" ? "trust" : r.status === "DECLINED" ? "neutral" : "alert"}" solid>${esc(REF_STATUS[r.status])}</ni-tag>`}</div><small class="dim">${esc(r.context)}</small></li>`).join("")}</ul>` : '<span class="dim">None yet.</span>', { desc: `${myRefs.filter((r: Referral) => r.status === "ACCEPTED").length} accepted into the network` })}</div></div>`;
    else if (tab === "privacy") {
      // This used to be one card in the bottom-right of Home. It is the thing a person in a
      // referral network cares most about, so it gets its own page and its own entry in the rail.
      const priv = C().privacy.privacyOf(p);
      body = `<div class="sect-head"><div><h2>Privacy &amp; control</h2><p>What we hold about you, who can see it, and how to take it back. Every switch here takes effect immediately.</p></div></div>
      <div class="grid-3"><div class="col-2 stack">
        ${privacyCard(p)}
        ${card("Your consent", `<form data-action="consent" data-id="${p.id}" class="stack"><div class="choice compact">${[["yes", "Refer me for opportunities"], ["ask", "Ask me each time"], ["no", "Not for now"]].map(([k, l]) => `<label class="${p.referralConsent === k ? "on" : ""}"><input type="radio" name="referralConsent" value="${k}" ${p.referralConsent === k ? "checked" : ""}><span>${esc(l)}</span></label>`).join("")}</div><div class="row end"><button class="btn glass sm" type="submit">Save</button></div></form>`, { desc: "Whether people in the network may put you forward. You are never named without being asked." })}
        ${card("What we hold, and for how long", `<div class="scroll"><table class="data"><thead><tr><th>What</th><th>Why we may hold it</th><th>Kept</th><th>Then</th></tr></thead><tbody>${C().privacy.RETENTION.map((r: any) => `<tr><td><b>${esc(r.label)}</b><small class="sub">${esc(r.what)}</small></td><td class="dim">${esc(r.ukBasis)} · ${esc(r.pdplBasis)}</td><td class="nowrap">${r.months ? `${r.months} mo` : "until you withdraw"}</td><td class="nowrap">${esc(r.onExpiry)}</td></tr>`).join("")}</tbody></table></div>`, { desc: "From the day of the trigger. Nothing is kept longer than this.", flush: true })}
      </div><div class="stack">
        ${card("Your rights", `<ul class="rows">${C().privacy.RIGHTS.map((r: any) => `<li class="col"><b>${esc(r.label)}</b><small class="dim">${esc(r.detail)}</small></li>`).join("")}</ul>`, { desc: "Each one is a button here, not a support ticket." })}
        ${card("Take it with you, or take it back", `<p class="dim">A copy of everything we hold about you, in a format anybody can read. Or ask us to delete it — we remove your profile and your conversation, and anonymise only the fee and audit records the law requires us to keep.</p><div class="row" style="margin-top:12px">${btn("Export my data", `data-act="exportMe" data-id="${p.id}"`, "glass sm")}${btn("Delete my profile", `data-act="eraseMe" data-id="${p.id}"`, "ghost sm")}</div>`)}
        ${card("Where your data lives", `<ul class="rows"><li class="col"><b>In the network</b><small class="dim">Your profile, your conversation and the vouches people gave you. Never pooled with another organisation.</small></li><li class="col"><b>On your own device</b><small class="dim">Anything you said aloud during screening. It became text locally; no audio was uploaded or stored.</small></li><li class="col"><b>Nowhere else</b><small class="dim">${priv.consents.shareAnonymised ? "Clients and agencies see an anonymised card only, until you agree to a specific role." : "You are not shown to clients or agencies at all."}</small></li></ul>`)}
      </div></div>`;
    }
      ...
      ${card("Privacy & control", `<p class="dim">${esc(C().privacy.privacySummary(p))}</p><p class="dim" style="margin-top:8px">Your permissions, what we hold, how long we keep it, and how to export or delete it — all in one place.</p><div class="row end" style="margin-top:12px"><a class="btn glass sm" href="#/member?tab=privacy">Open privacy &amp; control</a></div>`, { desc: "Nothing about you leaves the network without your say-so." })}
    ...
    const html = `<div class="member-hero"><div class="who">${avatar(p, "xl", true)}<div><div class="eyebrow">Network member${p.memberSince ? ` since ${esc(new Date(p.memberSince).toLocaleDateString("en-GB", { month: "short", year: "numeric" }))}` : ""}</div><h1>${esc(C().full(p))}</h1><p>${esc(p.headline ?? "Add a headline in your screening")}</p><div class="meta">${screenLabel(p)}${availBadge(p)}${p.referralConsent === "yes" ? badge("on the radar", "teal", true) : p.referralConsent === "no" ? badge("not being referred", "neutral", true) : badge("asked each time", "amber", true)}</div></div></div><div class="hero-stats"><ni-stat ghost label="Trust" value="${t.score}" note="${esc(C().trust.TRUST_BAND_LABEL[t.band])}"></ni-stat><ni-stat ghost label="Vouched by" value="${t.vouchedBy}" note="people who stand behind you"></ni-stat><ni-stat ghost label="Referrals" value="${myRefs.length}" note="${myRefs.filter((r: Referral) => r.status === "ACCEPTED").length} accepted"></ni-stat><ni-stat ghost label="Open to you" value="${open.length}" note="opportunities" href="#/member?tab=opportunities"></ni-stat></div></div>
      <nav class="tabs">${[["home", "Home"], ["opportunities", "Opportunities"], ["refer", "Refer someone"], ["privacy", "Privacy & control"]].map(([k, l]) => `<a href="#/member${k === "home" ? "" : `?tab=${k}`}" class="${tab === k ? "active" : ""}">${l}</a>`).join("")}</nav>${body}`;
