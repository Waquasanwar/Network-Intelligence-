...
import * as agreement from "@/lib/agreement";
import * as velocity from "@/lib/velocity";
...
async function boot() {
  ...
  const railNav = () => {
    ...
    if (role === "MEMBER") return `<nav>${[["/member", "My expert profile", "member"], ["/member?tab=opportunities", "Work open to me", "opportunities"], ["/member?tab=refer", "Refer someone", "referrals"]].map(link).join("")}<div class="group">Your settings</div>${[["/member?tab=privacy", "Privacy & control", "trust"]].map(link).join("")}</nav>`;
    if (role !== "OWNER") return `<nav>${[["/portal", "My requirements", "requirements"], ["/portal?tab=people", "Experts proposed", "portal"]].map(link).join("")}<div class="group">Your settings</div>${[["/portal?tab=terms", "Membership & terms", "money"]].map(link).join("")}</nav>`;
