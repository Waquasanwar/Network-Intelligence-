...
async function main() {
  ...
  const briefFrom = async (account: { id: string; kind: AccountKind }, text: string, over: { submittedVia?: "OWNER" | "PORTAL"; status?: "NEW" | "QUALIFYING" | "SEARCHING" | "SHORTLISTED" | "INTRODUCING" | "FILLED" | "CLOSED"; expertHours?: number; termsAccepted?: boolean; termsAcceptedAt?: Date } = {}) => {
  ...
  const b1 = await briefFrom(accounts.gulf, "Gulf Talent Partners is looking for 2 BAs already in Dubai with a visa, perm, retail banking, AED 360k, start asap", { submittedVia: "PORTAL", status: "SHORTLISTED" });
  ...
  const b4 = await briefFrom(accounts.meridian, "Fractional operations lead 2 days a week to build the Dubai operating model, £1,700/day, 9 months, must be willing to spend time in Dubai", { termsAccepted: true, termsAcceptedAt: daysAgo(10) });
  ...
  console.log(`Seeded ${PEOPLE.length} people, 6 opportunities, 4 requirements, 3 tenants.`);
