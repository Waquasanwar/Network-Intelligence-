...
import { portalSubmitBrief, portalRespond, portalAcceptTerms, resolvePortalAccount, toDomain, loadRateCard } from "@/server/actions/demand";
                ...
                <CardBody>
                  ...
                  {items.length === 0 ? <div className="text-xs text-ink-muted">{b.status === "NEW" ? "Received. We are reading it now." : "Searching the network. You will see anonymised cards here first."}</div> : (
                    <ul>{items.map((s) => {
                      ...
                      return <AnonCard key={s.id} refCode={safe.ref} headline={safe.headlineSummary || "Profile"} region={safe.region} availabilityBand={safe.availabilityBand} evidenceSummary={safe.evidenceSummary} rights={checks.find((c) => c.label === "Work rights")} rateStated={s.person.rateExpectation ?? s.person.salaryExpectation} tier={s.tier} decision={s.decision} clientNote={s.clientNote} revealedName={revealed ? fullName(s.person) : null}
