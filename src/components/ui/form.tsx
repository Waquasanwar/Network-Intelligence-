...
const base =
  "w-full rounded-[10px] border border-line bg-surface px-3 text-[13px] text-ink placeholder:text-ink-faint shadow-[0_1px_1px_rgba(16,24,40,0.03)] transition-[border-color,box-shadow] duration-100 hover:border-line-strong focus:outline-none focus:border-navy-400/60 focus:ring-3 focus:ring-navy-400/15 disabled:opacity-60 disabled:bg-surface-muted";
...
export const Select = React.forwardRef<HTMLSelectElement, React.SelectHTMLAttributes<HTMLSelectElement>>(({ className, children, ...props }, ref) => (
  <select ref={ref} className={cn(base, "h-9 pr-7 appearance-none bg-[url('data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2212%22 height=%2212%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%238d93a0%22 stroke-width=%222%22><path d=%22m6 9 6 6 6-6%22/></svg>')] bg-no-repeat bg-[right_8px_center]", className)} {...props}>
    {children}
  </select>
));
